...
function processHtmlContent($content)
{
    ...
    $escapedContent = str_replace('"', '\"', $decodedContent);
...
function testdoc($query)
{
 $encodedQuery = urlencode($query);
	...
	for ($i = 0; $i < $divsWithLink->length; $i++) {
		...
		$xinder .= ResultGen($res,$id+1,processHtmlContent(generateTitleFromParagraph($description)));
		$xinder .= SummaryGen($id+2,$id+1,processHtmlContent("<p>".$description."</p>"));
		...
		echo $description."</br>";
		echo "<b>Images</b></br>";
		$images = returnImageResults($description);
		echo $images;
		$xinder .= ImageGen($id+3,$id+1,processHtmlContent($images));
		echo "<br>";
		echo "<b>Videos</b></br>";
		set_time_limit(50);
		...
		$vids = "";
		if (isset($urls[0])) {$vids = $vids.generateresult($urls[0]);}
		if (isset($urls[1])) {$vids = $vids.generateresult($urls[1]);}
		if (isset($urls[2])) {$vids = $vids.generateresult($urls[2]);}
		if (isset($urls[3])) {$vids = $vids.generateresult($urls[3]);}
		if (isset($urls[4])) {$vids = $vids.generateresult($urls[4]);}
		echo $vids;
		$xinder .= VideoGen($id+4,$id+1,processHtmlContent($vids));
		$id=$id+4;
		$res=$res+1;
	...
	fwrite($fp, $xinder) or die("Couldn't write values to file!"); 
	fclose($fp); 
	//echo "JSON";
	//echo $xinder;
}
...
function returnTextResults($query)
{
    ...
    $divs = $xpath->query("//div[@id='main']");
    $divs2 = $xpath->query("//div[contains(.,'People also ask') or contains(.,'Related searches') or contains(.,'Images')]");
